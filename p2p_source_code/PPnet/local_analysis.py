...
log, logclose = create_logger(log_filename=os.path.join(
    save_analysis_path, 'local_analysis.log'))
...
ppnet = torch.load(load_model_path, map_location=torch.device('cuda'))
ppnet = ppnet.module
ppnet = ppnet.cuda()
...
def save_prototype(fname, epoch, index):
    p_img = plt.imread(os.path.join(load_img_dir, 'epoch-' +
                                    str(epoch), f'{(5 - len(str(index))) * "0"}{str(index)}_prototype-img'+'.png'))
    # plt.axis('off')
...
def save_prototype_original_img_with_bbox(fname, epoch, index,
    ...
    p_img_bgr = cv2.imread(os.path.join(
        load_img_dir, 'epoch-'+str(epoch), f'{(5 - len(str(index))) * "0"}{str(index)}_prototype-img-original'+'.png'))
...
prototype_activations = ppnet.distance_2_similarity(min_distances[0])
...
for i in range(1, 11):
    ...
    save_prototype_original_img_with_bbox(fname=os.path.join(save_analysis_path, 'most_activated_prototypes',
                                          ...
                                          bbox_height_start=prototype_info[sorted_indices_act[-i].item(
                                          )][1],
                                          bbox_height_end=prototype_info[sorted_indices_act[-i].item(
                                          )][2],
                                          bbox_width_start=prototype_info[sorted_indices_act[-i].item(
                                          )][3],
                                          bbox_width_end=prototype_info[sorted_indices_act[-i].item(
                                          )][4],
                                          color=(0, 255, 255))
    ...
    activation_pattern = prototype_activation_patterns[idx][sorted_indices_act[-i].item(
    )].detach().cpu().numpy()
    ...
    rescaled_activation_pattern = upsampled_activation_pattern - \
...
for i, c in enumerate(topk_classes.detach().cpu().numpy()):
    ...
    for j in reversed(sorted_indices_cls_act.detach().cpu().numpy()):
        ...
        activation_pattern = prototype_activation_patterns[idx][prototype_index].detach(
        ).cpu().numpy()
        upsampled_activation_pattern = cv2.resize(activation_pattern, dsize=(img_size, img_size),
                                                  interpolation=cv2.INTER_CUBIC)
