...
#                            vmax=1.0)
                ...
                if patch.patch.shape[0] != img_size or patch.patch.shape[1] != img_size:
                    np.save(os.path.join(dir_for_saving_images,
                                         "%02d_" % (i+1) + 'nearest-' + '_receptive_field_indices.npy'),
                            patch.patch_indices)
                    plt.imsave(fname=os.path.join(dir_for_saving_images,
                    ...
                    plt.imsave(fname=os.path.join(dir_for_saving_images,
                                              "%02d_" % (i+1) + 'nearest-' + '_receptive_field_with_heatmap.png'),
                               arr=overlayed_patch,
                               vmin=0.0,
                               vmax=1.0)
...
#                 np.save(os.path.join(dir_for_saving_images,
#                                      "%02d_" % (i+1) + 'nearest-' + '_high_act_patch_indices.npy'),
#                         high_act_patch_indices)
                ...
                imsave_with_bbox(fname=os.path.join(dir_for_saving_images,
                                       "%02d_" % (i+1) + 'nearest' + '_high_act_patch_in_original_img.png'),
