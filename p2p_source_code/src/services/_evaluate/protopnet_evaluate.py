...
from lib.utils import evaluate as eval_acc
...
class Service(object):
    ...
    def __call__(self):
        ...
        teacher = "teacher resnet50"
        kd = "kd student resnet18"
        student = "baseline student resnet18"

        print("Start to evaluate the Accuracy on Test Datasets:")

        self.evaluate_accuracy(teacher, self.teacher_model,self.dataset_loader.test_loader)

        self.evaluate_accuracy(kd, self.student_kd_model,self.dataset_loader.test_loader)

        self.evaluate_accuracy(student, self.student_baseline_model,self.dataset_loader.test_loader)

        # assert manager.common.testBatchSize == 1

        print("Start to evaluate the AAPs, AJS and PMS on Test Datasets:")
    ...
    def evaluate_accuracy(self, model_name, our_model, loader ):
        result = eval_acc.evaluate_model(our_model, loader, mgpus=self.mgpus)
